...
# Configure the KML directory path
...
Speed_DATA_DIR = r"Y:\Data Analysis Team\Wind tool\data_harsh"

# Or even better, make it configurable:
# Speed_DATA_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "data"))
...
def get_mast():
    ...
    try:
        ...
        print(f"Looking for CSVs in: {state_dir}")  # Debug
        ...
        # Get CSV files (case-insensitive, exclude hidden files)
        csv_files = []
        for f in os.listdir(state_dir):
            if f.lower().endswith('.csv') and not f.startswith('.'):
                csv_files.append(os.path.join(state_dir, f))
        
        print(f"Found {len(csv_files)} CSV files")  # Debug
        ...
        # Extract mast names (filename without extension)
        masts = [os.path.splitext(os.path.basename(f))[0] for f in csv_files]
        print(f"Masts found: {masts}")  # Debug
        ...
        print(f"Error in get_mast: {str(e)}")  # Debug
        return jsonify({
            'success': False,
...
def get_column():
    ...
    try:
        ...
        # Filter speed columns (starts with 'Spd' and ends with '[m/s]')
        speed_columns = [col for col in df.columns 
                        if re.match(r'^Spd.*\[m/s\]$', col, re.IGNORECASE)]
        
        # Filter direction columns (starts with 'Dir' and contains degree symbol)
...
def process_speed_analysis(start_dt, end_dt, selections):
    ...
    # For each selection, process the data
    for selection in selections:
        ...
        try:
            # Try reading with different timestamp formats
            try:
                df = pd.read_csv(csv_path)
                # Try different timestamp column names and formats
                timestamp_col = None
                ...
                # Add values to monthly data in order (Jan-Dec)
                for month_num in range(1, 13):
                    month_avg = monthly_avg.get(month_num, 0)  # Default to 0 if no data
            ...
            # If there's an error with one file, skip it but continue with others
            print(f"Error processing {state}/{mast}: {str(e)}")
...
def process_direction_analysis(start_dt, end_dt, selections, sector_count):
    ...
    # Create sector labels with ranges (e.g., "0°-30°")
    ...
    for selection in selections:
        ...
        try:
            ...
            try:
                ...
                # Replace 9999 with NaN in direction column
            ...
            # If there's an error with one file, skip it but continue with others
            print(f"Error processing {state}/{mast}: {str(e)}")
            # Add zero frequencies for all sectors
            sector_frequencies = []
            for sector in range(sector_count):
                midpoint = sector * sector_angle + (sector_angle / 2)
                sector_frequencies.append({
...
BASE_DIR = r"Y:\Data Analysis Team\Wind tool"
...
# Dictionary to store extrapolated data in memory
extrapolated_data = {}
...
# Precompute months list for Excel generation
MONTHS = ["January", "February", "March", "April", "May", "June", 
          "July", "August", "September", "October", "November", "December", "Average"]

# Excel preparation queue and cache
...
def get_safe_chunksize(file_size):
    ...
    try:
        available_mem = psutil.virtual_memory().available * 0.8  # Use 80% of available memory
        estimated_mem_per_chunk = file_size / 100000  # Estimate memory per chunk
...
def update_total_loss_factor():
    ...
    try:
        ...
        total_loss_factor = 0.8  # Default fallback
...
def is_relevant_height(col_name):
    ...
    return True  # Now returns True for all heights
    ...
    try:
        ...
        if date_range['type'] in ['yearly', 'date_range', 'average']:
            ...
            for (year, month), month_df in grouped:
                ...
                # Batch power calculation for the month
...
def process_data(state, mast, speed_col, power_curve_name, date_range):
    ...
    try:
        ...
        # Load power curve first (fail fast if invalid)
        ...
        # Get file size for chunksize calculation
        ...
        # First pass: Identify complete years for average mode
        complete_years = set()
        if date_range['type'] == 'average':
            ...
            for chunk in pd.read_csv(mast_path, chunksize=chunksize):
                try:
                    ...
                    # Consider a year complete if it has data for all 12 months
        ...
        for chunk_idx, chunk in enumerate(pd.read_csv(mast_path, chunksize=chunksize)):
            try:
                ...
                # For average mode, filter to only complete years
...
def calculate_data_sheet_stats(state, mast, date_range, power_curves):
    ...
    try:
        ...
        for chunk in pd.read_csv(file_path, chunksize=chunksize):
            try:
                ...
                for group_key, month_df in grouped:
                    ...
                    # Calculate mean wind speed for all speed columns
                    ...
                    # Calculate energy pattern factor for all heights
                    for col in speed_cols:
                        clean_name = clean_column_name(col)
                    ...
                    # Calculate power output and energy for each power curve and relevant heights (>=140m)
        ...
        # Calculate averages for parameters
...
def create_excel_data_sheet(data, state, mast, date_range):
    ...
    try:
        ...
        # Group data by year first (excluding yearly summaries which have month=13)
        yearly_data = defaultdict(list)
        for entry in data:
            if entry and entry.get('month') != 13:  # Added check for None entry
                yearly_data[entry['year']].append(entry)
        
        # For average mode, we need to calculate averages across all years
        if date_range['type'] == 'average' and yearly_data:
            # Create a synthetic "average" entry by combining all years
            avg_entries = []
            all_months_data = defaultdict(list)
            
            # Collect all data for each month across all years
            ...
            for month in range(1, 13):
                ...
                aggregated = {
                    'year': 9999,  # Special year value for average
                    'month': month,
                ...
                # Calculate averages for energy data
                final_energy = []
            ...
            # Add the average entries to yearly_data
            yearly_data[9999] = avg_entries
        
        # Process years in sorted order
        current_row = 4  # Start after the header rows
        
        for year in sorted(yearly_data.keys()):
            year_entries = yearly_data[year]
            
            # Add year header (or "Average" for synthetic year)
            if year == 9999:
            ...
            # Process Mean Wind Speed for this year
            ...
            # Get all speed columns from this year's data
            speed_cols = []
            ...
            for height_dir in speed_cols_sorted:
                ...
                for i, month in enumerate(MONTHS[:-1]):
                    ...
                    # For average year, we have data for all months
                    if year == 9999:
            ...
            # Sort direction columns by height
            ...
            # Process Energy Pattern Factor - now for all heights
            ...
            # Get all EPF columns (removed height >= 140 condition)
            epf_cols = []
            if year_entries and year_entries[0].get('parameters'):
                for param in year_entries[0]['parameters'].keys():
            ...
            # Get all power curves and heights from energy data
            power_curves = set()
            heights = set()
        ...
        try:
            ...
            with excel_preparation_lock:
                if cache_key not in excel_preparation_cache:
                    ...
                    # If data wasn't provided, get it from cache
                    if data is None:
                        power_curves = ['suzlon']  # Default power curve, adjust as needed
...
# Start multiple worker threads when the app starts
NUM_EXCEL_WORKERS = 4
excel_threads = []
...
def modi():
    ...
    return render_template('modi.html', states=states, power_curves=power_curves)
...
def process():
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            # Create processing tasks for both results and data sheets
            for site in selected_sites:
                for pc in power_curves:
                    ...
                    # Also prepare data sheet in parallel
                    futures.append(executor.submit(
            ...
            # Queue Excel preparation in background immediately with None data
            # The worker will fetch the data when it's ready
            for site in selected_sites:
                excel_preparation_queue.put((
                    site['state'],
                    site['mast'],
                    date_range,
                    None  # Data will be filled later
                ))
            ...
            for future in as_completed(futures, timeout=300):  # 5 minute timeout
                try:
                    ...
                    if result:
                        if 'monthly_stats' in result:  # It's a process_data result
                            results.append(result)
                        else:  # It's a data sheet result
                            site_key = f"{result[0]['state']}_{result[0]['mast']}" if result else None
                            if site_key:
                                data_sheet_results[site_key] = result
                                # Update Excel preparation queue with actual data
                                excel_preparation_queue.put((
                                    result[0]['state'],
                                    result[0]['mast'],
                                    date_range,
...
def download_data_sheet():
    try:
        ...
        if 'date_range' in data:
            ...
            # Construct date_range from individual parameters
            date_range = {
        ...
        # Check if we have a pre-prepared Excel file
        cache_key = f"excel_{state}_{mast}_{date_range['type']}_{date_range.get('value', '')}_{date_range.get('start', '')}_{date_range.get('end', '')}"
        ...
        with excel_preparation_lock:
            if cache_key in excel_preparation_cache:
                ...
                # Fallback to on-demand generation if not pre-prepared
...
def cleanup():
    for _ in range(NUM_EXCEL_WORKERS):
        excel_preparation_queue.put(None)  # Signal workers to stop
    for t in excel_threads:
        t.join()

if __name__ == '__main__':
    freeze_support()
    # Create the KML directory if it doesn't exist
